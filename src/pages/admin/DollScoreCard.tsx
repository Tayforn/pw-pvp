...
export default function DollScoreCard({ draft, patch }: { draft: GearRules; patch: (p: Partial<GearRules>) => void }) {
  ...
  return (
    <div className="card" style={{ padding: 14 }}>
      ...
      <p className="hint" style={{ margin: '4px 0 6px' }}>
        Курс один для ПА і ПЗ: 1 ПА = 1 ПЗ = {weaponPzRate(draft).toFixed(2)} бала (бали за камінь Лагеря з картки «Лялька» ÷ 2 ПЗ). ПА на основній зброї — за цим
        курсом понад ПА зброї еталона; в атаку ляльки вона не входить, щоб не рахувати двічі. Свап ПЗ-зброя — за кожну одиницю ПЗ, яку дає заміна зброї
        ({weaponPaRate(draft).toFixed(2)} бала), але не більше стелі.
      </p>
