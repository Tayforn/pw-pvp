// =========================================================
// Сайдбар: статичні пункти (Головна/Турніри/Реєстрація) + динамічний
// пункт на кожну активну регулярну серію + Адмінка (лише для адміна).
// =========================================================
...
const seriesIco = <svg {...S}><circle cx="12" cy="12" r="8.5" /><path d="M12 7v5l3.5 2" /></svg>;
...
export default function Sidebar({ route, isAdmin, series, onNavigate }: Props) {
  const items: NavEntry[] = [
    ...
    ...series.filter((s) => s.isActive).map((s) => ({ route: { name: 'series' as const, slug: s.slug }, label: s.name, ico: seriesIco })),
  ];
