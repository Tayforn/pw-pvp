...
## Відомі помилки calc, свідомо збережені

Ядро зобовʼязане рахувати як незмінений calc (це й доводять golden-тести), тож
помилки calc тут не виправляються поодинці — лише разом у calc і pvp, з новою
версією ядра та перегенерованими еталонами.

- **Знак станів співу в «Час співу».** `summary.ts` і `derived.ts` рахують
  `ci − re + xj`, хоча `re` за текстом fustate «увеличивает скорость подготовки
  заклинаний» (як `ci` з речей), а `xj` — «уменьшает». Правильно було б
  `ci + re − xj`. На дельти сетів і майбутні бали не впливає (вони без бафів, а
  в речах `re`/`xj` немає); панель характеристик з увімкненими станами співу
